...
pub const RACE_RESULT_SIZE: usize = 32;
...
pub mod horse_racing {
    ...
    pub fn mint_nft(
        ...
        if let Some(sol_price) = sol_price {
            let rand_from_sol = sol_price as u64 + ctx.accounts.clock.unix_timestamp as u64;
        ...
        if let Some(btc_price) = btc_price {
            let rand_from_sol = btc_price as u64 + ctx.accounts.clock.unix_timestamp as u64;
        ...
        add_nft(ctx.accounts.mint.key(), ctx.accounts.nft_list)?;
    ...
    pub fn start_race(
        ...
        random_value += ctx.accounts.clock.unix_timestamp as u64;

        let nft_count = get_nft_count(ctx.accounts.nft_list)?;
        let winner_idx = random_value % nft_count as u64;
        msg!("winner is {} th horse!", winner_idx);

        // getting Pubkey of winner
        let winner_pk = get_winner(winner_idx, ctx.accounts.nft_list)?;
...
#[derive(Accounts)]
#[instruction(nft_list_bump: u8, operator_list_bump: u8, race_bump: u8)]
...
#[derive(Accounts)]
#[instruction(metadata_bump : u8, nft_list_bump: u8)]
...
pub fn add_nft<'info> (
    ...
    let count = get_nft_count(nft_list)?;
...
pub fn get_nft_count<'a> (
    ...
    let mut nft_list_data = nft_list.data.borrow();
    let count: u16 = u16::try_from_slice(&nft_list_data[0..2])?;
    Ok(count)
}

pub fn increase_nft_count<'a> (
    nft_list: AccountInfo<'a>
) -> ProgramResult {
    let mut nft_list_data = nft_list.data.borrow_mut();
    let count: u16 = u16::try_from_slice(&nft_list_data[0..2])?;
